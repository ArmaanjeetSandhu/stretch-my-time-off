...
const isWeekend = (date: Date, weekendDays: number[]): boolean =>
  weekendDays.includes(date.getDay());
const isHoliday = (date: Date, holidays: { date: Date }[]): boolean =>
  holidays.some((h) => dateKey(h.date) === dateKey(date));
const daysBetween = (start: Date, end: Date): number =>
  Math.round((end.getTime() - start.getTime()) / MS_IN_A_DAY);
...
export function getHolidaysForYear(
  ...
  const hd = stateCode
    ? new Holidays(countryCode, stateCode, opts)
    : new Holidays(countryCode, opts);
      ...
      Array.from(
        ...
        (_, i) => ({
          date: new Date(
            ...
            holiday.start.getMonth(),
            holiday.start.getDate() + i,
          ),
          name: holiday.name,
        }),
      ),
...
export function optimizeDaysOff(
  ...
  const filteredHolidays = holidays.filter(
    (h) => h.date.getFullYear() === year && h.date >= effectiveStartDate,
  );
  const filteredFixedDaysOff = fixedDaysOff.filter(
    (d) => d.getFullYear() === year && d >= effectiveStartDate,
  );
  const allDaysOff = new Set([
    ...filteredHolidays.map((h) => dateKey(h.date)),
    ...filteredFixedDaysOff.map((d) => dateKey(d)),
    ...getWeekends(year, weekendDays, effectiveStartDate).map((d) =>
      dateKey(d),
    ),
  ]);

  const gaps = findGaps(allDaysOff, year, weekendDays, effectiveStartDate);
  return selectDaysOff(
    rankGapsByEfficiency(gaps, allDaysOff, weekendDays),
    daysOff,
    allDaysOff,
    weekendDays,
  );
...
export function calculateConsecutiveDaysOff(
  ...
  const filteredOptimizedDaysOff = optimizedDaysOff.filter(
    (d) => d >= effectiveStartDate,
  );
  const filteredFixedDaysOff = fixedDaysOff.filter(
    (d) => d >= effectiveStartDate,
  );
  ...
  for (
    ...
    if (
      isWeekend(d, weekendDays) ||
      isHoliday(d, filteredHolidays) ||
      allDaysOff.has(dateKey(d))
    ) {
      ...
      if (isValidConsecutiveGroup(currentGroup, weekendDays)) {
        consecutiveDaysOff.push(
          createPeriod(
            currentGroup,
            filteredOptimizedDaysOff,
            filteredFixedDaysOff,
          ),
        );
      }
      currentGroup = [];
  ...
  if (
    ...
    isValidConsecutiveGroup(currentGroup, weekendDays)
  ) {
    consecutiveDaysOff.push(
      createPeriod(
        currentGroup,
...
function getWeekends(
  ...
  for (
    ...
    if (isWeekend(d, weekendDays)) weekends.push(new Date(d));
  }
  return weekends;
}
...
function findGaps(
  ...
  const maxGapLength = Math.max(1, 7 - new Set(weekendDays).size);
  const gaps = [];
  let gapStart = null;
  ...
  return gaps
    .map((gap) => {
      const [backward, forward] = ["backward", "forward"].map((direction) =>
        calculateChain(
          direction === "backward" ? gap.start : gap.end,
          gap.gapLength,
          allDaysOff,
      ...
      return forward.chainLength > backward.chainLength ||
        (forward.chainLength === backward.chainLength &&
          forward.usedDaysOff <= backward.usedDaysOff)
...
function calculateChain(
  ...
  while (
    allDaysOff.has(dateKey(neighbor)) ||
    isWeekend(neighbor, weekendDays)
  ) {
    chainLength++;
...
function isValidConsecutiveGroup(
  group: Date[],
  weekendDays: number[],
): boolean {
  if (group.length < 2) return false;

  const allDaysAreWeekends = group.every((d) =>
...
function createPeriod(
  ...
  return {
    ...
    usedDaysOff: group.filter((d) =>
      optimizedDaysOff.some((od) => dateKey(od) === dateKey(d)),
    ).length,
  };
}
