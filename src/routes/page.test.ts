...
function isAlreadyDayOff(
  ...
  const isHolidayDay = holidays.some(
    (h) =>
      h.date.getFullYear() === date.getFullYear() &&
      h.date.getMonth() === date.getMonth() &&
      h.date.getDate() === date.getDate(),
  );
  const isOptimizedDay = optimizedDaysOff.some(
    (d) =>
...
function simulateToggleFixedDayOff(
  ...
  const isAlreadyDayOffDay = isAlreadyDayOff(
    normalizedDate,
    weekendDays,
    holidays,
    optimizedDaysOff,
  ...
  const existingIndex = fixedDaysOff.findIndex(
    (d) => dateKey(d) === dateKeyStr,
  );
  ...
  if (existingIndex >= 0)
    ...
    if (!isAlreadyDayOffDay) newDaysOff = daysOff + 1;
  }
...
describe("Fixed Days Off Logic", () => {
  ...
  describe("toggleFixedDayOff behavior", () => {
    it("should increase days off when adding a fixed day off to a regular day", () => {
      const regularDay = new Date(TEST_YEAR, 0, 15);
    ...
    it("should NOT increase days off when adding a fixed day off to an optimized day", () => {
      const optimizedDay = new Date(TEST_YEAR, 0, 15);
      const fixedDaysOff: Date[] = [];
      const daysOff = 10;
      const holidays: Array<{ date: Date; name: string }> = [];
    ...
    it("should handle multiple fixed days off correctly", () => {
      ...
      // Add first day
      let result = simulateToggleFixedDayOff(
        day1,
        fixedDaysOff,
        daysOff,
        WEEKEND_DAYS,
      ...
      // Add second day
      result = simulateToggleFixedDayOff(
        day2,
        fixedDaysOff,
        daysOff,
        WEEKEND_DAYS,
